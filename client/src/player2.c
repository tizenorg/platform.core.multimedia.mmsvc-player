...
int player_create(player_h *player)
{
	...
	if (ret == PLAYER_ERROR_NONE) {
		...
		*player = (player_h)pc;
		...
		if(strncmp(ini.videosink_element_x, "fakesink", strlen("fakesink")) == 0) {
		pc->have_wlclient = TRUE;
		LOGE(" have wlclient ");
		}
#endif
		pc->have_wlclient = TRUE;
		LOGE("have wlclient (%d)", pc->have_wlclient);
#endif
		if (!pc->have_wlclient)
		{
...
int player_destroy(player_h player)
{
	...
	if (CALLBACK_INFO(pc)) {
		if (!pc->have_wlclient && mm_player_mused_destroy(INT_HANDLE(pc)) != MM_ERROR_NONE)
			ret = PLAYER_ERROR_INVALID_OPERATION;
...
int player_unprepare(player_h player)
{
	...
	if (!pc->have_wlclient)
	{
		...
		ret = mm_player_wlclient_finalize(pc->wlclient);
...
int player_set_display_mode(player_h player, player_display_mode_e mode)
{
	...
	{
		if(pc->wlclient->caps) {
			LOGD("need to send meta buf");
			pc->wlclient->display_geo_method = mode;
			ret = mm_player_wlclient_send_meta_buf (pc->wlclient, NULL);
		}
		return ret;
	}
...
int player_set_display_rotation(player_h player, player_display_rotation_e rotation)
{
	...
	if (pc->have_wlclient)
	{
		if(pc->wlclient->caps) {
			LOGD("need to send meta buf");
			pc->wlclient->rotate_angle = rotation;
			ret = mm_player_wlclient_send_meta_buf (pc->wlclient, NULL);
		}
		return ret;
	}
