...
#if 0
	unsigned upper_offset;
	unsigned lower_offset;

	if(player_msg_get(upper_offset, recvMsg)
			&& player_msg_get(lower_offset, recvMsg)) {
		offset = (((unsigned long long)upper_offset << 32) & 0xffffffff00000000)
			| (lower_offset & 0xffffffff);
...
int player_create(player_h * player)
{
	...
	pc = g_new(player_cli_s, 1);
	if (pc == NULL) {
		ret = PLAYER_ERROR_OUT_OF_MEMORY;
		goto ErrorExit;
	}
	memset(pc, 0, sizeof(player_cli_s));
...
#if 0
	if(ret == PLAYER_ERROR_NONE) {
		if(mm_player_start(INT_HANDLE(pc)) != MM_ERROR_NONE)
			ret = PLAYER_ERROR_INVALID_OPERATION;
	}
#endif
...
int player_stop(player_h player)
{
	...
	//mm_ret = mm_player_stop(INT_HANDLE(pc));
		...
		player_stream_type_e type, unsigned long long max_size)
{
	...
	char *ret_buf = NULL;
#if 0
	unsigned upper_max_size = (unsigned)((max_size >> 32) & 0xffffffff);
	unsigned lower_max_size = (unsigned)(max_size & 0xffffffff);
#endif
	...
	player_msg_send3(api, EXT_HANDLE(pc), sock_fd, pc->cb_info, ret_buf, ret,
			INT, type, INT, upper_max_size, INT, lower_max_size);
#else
	player_msg_send2(api, EXT_HANDLE(pc), sock_fd, pc->cb_info, ret_buf, ret,
		...
		player_stream_type_e type, unsigned long long *pmax_size)
{
	...
	char *ret_buf = NULL;
#if 0
	unsigned upper_max_size;
	unsigned lower_max_size;
#else
	...
	if(ret == PLAYER_ERROR_NONE){
#if 0
		player_msg_get(upper_max_size, ret_buf);
		player_msg_get(lower_max_size, ret_buf);
		*pmax_size = (((unsigned long long)upper_max_size << 32) & 0xffffffff00000000)
