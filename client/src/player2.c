...
int player_set_audio_policy_info(player_h player, sound_stream_info_h stream_info)
{
	...
	int ret = __player_convert_error_code(sound_manager_is_available_stream_information(stream_info, NATIVE_API_PLAYER, &is_available)
										  , (char *)__FUNCTION__);
...
int player_set_display(player_h player, player_display_type_e type, player_display_h display)
{
	...
	if (type != PLAYER_DISPLAY_TYPE_NONE) {
		...
		if (object_type) {
			if (type == PLAYER_DISPLAY_TYPE_OVERLAY && !strcmp(object_type, "elm_win")) {
				...
				LOGI("xid %d, surface_id %d, surface %p(%d), win_id %d", elm_win_xwindow_get(obj), ecore_wl_window_surface_id_get(wl_window), ecore_wl_window_surface_get(wl_window), *(int *)ecore_wl_window_surface_get(wl_window), ecore_wl_window_id_get(wl_window));
#else
	...
	if (CALLBACK_INFO(pc)) {
		ret = mm_player_set_attribute(INT_HANDLE(pc), NULL, "display_surface_type", type, "pipeline_type", mmPipelineType, "wl_display", set_wl_display, sizeof(void *), "display_overlay", set_handle, sizeof(display), (char *)NULL);
		if (ret != MM_ERROR_NONE)
			LOGE("Failed to display surface change :%d", ret);

		ret = mm_player_set_attribute(INT_HANDLE(pc), NULL, "wl_window_render_x", wl_win.wl_window_x, "wl_window_render_y", wl_win.wl_window_y, "wl_window_render_width", wl_win.wl_window_width, "wl_window_render_height", wl_win.wl_window_height, (char *)NULL);
