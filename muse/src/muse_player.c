...
static tbm_key _create_export_data (muse_player_handle_s *muse_player, void *data, int size, void *ext_data, int ext_size)
{
	...
	if (ext_size > 0) {
		memcpy(thandle.ptr+size, ext_data, ext_size);
	}
	tbm_bo_unmap(bo);
	...
	LOGD("bo %p, vaddr %p, size %d(%d, %d), key %d", bo, thandle.ptr, bo_size, size, ext_size, key);
...
static void _audio_frame_decoded_cb(player_audio_raw_data_s * audio_frame, void *user_data)
{
	...
	/* send message */
	info_size = sizeof(player_audio_raw_data_s);
	player_msg_event3(api, ev, module, INT, key, INT, size, INT, info_size);
	return;
}
...
int player_disp_set_gapless(muse_module_h module)
{
	int ret = PLAYER_ERROR_NONE;
...
int player_disp_is_gapless(muse_module_h module)
{
	int ret = PLAYER_ERROR_NONE;
